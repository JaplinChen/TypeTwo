...
| M5 發版與維運閉環 | 本機完成 | CI backend-quality、migration gate、Docker build、smoke、release backend artifact、installer artifact、checksum、rollback runbook 與備份還原驗證已完成 |
...
| GitHub Actions 綠燈 | 需要 push 到 GitHub 觸發雲端 workflow | `CI / backend-quality` 與 `CI / windows-quality` 皆通過 |
| release artifact 可下載 | 需要建立 GitHub Release 或手動 workflow_dispatch | Release 內有 `setup_typetwo.exe`、`setup_typetwo.exe.sha256`、`typetwo-glossary-api-<version>.tar`、`typetwo-glossary-api-<version>.tar.sha256` |
...
4. GitHub Actions `backend-quality` 與 `windows-quality` 綠燈。
5. GitHub Release 產出 installer、backend image tar 與各自 checksum。
